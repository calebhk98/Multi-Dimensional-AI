...
class TestTransformerMasking:
    ...
    def test_backbone_runs_with_padding_mask(self, model_config):
        """
        Purpose:
            Ensure the backbone runs without error when a padding mask is provided.
            This validates the fix for the 4D mask crash.

        Workflow:
            1. Initialize a TransformerBackbone with a standard config.
            2. Create input tensor and a corresponding attention mask (1s for valid, 0s for padding).
            3. Pass inputs to the model.
            4. Verify output shape matches input shape and contains no NaNs.
    ...
    def test_padding_is_effectively_ignored(self, model_config):
        """
        Purpose:
            Ensure that padding tokens do not affect the output of valid tokens.
            If attention is properly masked, changing the content of padded tokens should not change 
            the output valid tokens.

        Workflow:
            1. Initialize a single-layer TransformerBackbone.
            2. Create input tensor and a mask with padding.
            3. Run forward pass (output1).
            4. Modify the *padded* values in the input tensor significantly.
            5. Run forward pass again (output2).
            6. Assert that the outputs for the *valid* tokens are identical (within tolerance).
    ...
    def test_key_padding_mask_propagation(self):
        """
        Purpose:
            Verify parameters are passed correctly to the underlying layer.
        
        Workflow:
            1. Initialize a TransformerLayer.
            2. Run forward pass with no mask (Case 1).
            3. Run forward pass with explicit key_padding_mask (Case 2).
            4. Verify output shape is correct in both cases.

        ToDo:
            - Add more rigorous check for mask alignment.
        """
